...
Status: adapter `0.1.6` adds one pure typed operation that adopts the exact
non-secret request evidence produced by RED-CMS core P3E-9D0. It does not add
or expose the real provider operation.
...
Version `0.1.6` adds byte-identical source and package copies of
...
passes 62 focused assertions proving exact package identity/integrity,
...
provider operation. The aggregate adapter suite passes 1,061 assertions while
